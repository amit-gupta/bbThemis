...
PageCache page_cache(PageCache::ConsistencyLevel::VISIBLE_AFTER_WRITE);
...
uint64_t readFile(const char *filename, double &metadata_sec, double &read_sec) {
  ...
  timer = MPI_Wtime();
  int fd = page_cache.open(filename, O_RDONLY);
...
long writeFile(const char *filename, const long size,
  ...
  timer = MPI_Wtime();
  int fd = page_cache.open(filename, O_CREAT | O_WRONLY);
  metadata_sec += MPI_Wtime() - timer;
  if (fd < 0) {
  ...
  while (total_bytes_written < size) {
    ...
    ssize_t bytes_written = page_cache.write(fd, buffer, write_len);
    if (bytes_written < 0) {
...
long alignedRead(const vector<StridedContent> &content_list, double &meta_sec, double &data_sec) {
  ...
  for (const StridedContent &sc : content_list) {
    ...
    while (pos < sc.file_size) {
      ...
      ssize_t bytes_read = page_cache.pread(fd, buf.data(), read_len, pos);
...
long alignedWrite(const vector<StridedContent> &content_list, double &meta_sec, double &data_sec) {
  ...
  for (const StridedContent &sc : content_list) {
    ...
    while (pos < sc.file_size) {
      ...
      ssize_t bytes_written = page_cache.pwrite(fd, buf.data(), write_len, pos);
      if (bytes_written < 0) {
