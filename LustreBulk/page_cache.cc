...
//        std::cout<<"opened file "<<open_file->canonical_path<<" fd="<<fd<<" mode"<<open_file->access<<std::endl;        
      } else {
...
//        std::cout<<"opened file "<<open_file->canonical_path<<" fd="<<fd<<" mode"<<open_file->access<<std::endl;
  ...
  if (fd_iter != file_fds.end()) {
    fprintf(stderr, "Error opening %s: got file descriptor %d, which is in use %s\n",
            pathname, fd_iter->second->fd, fd_iter->second->path);
  } else {
